defmodule PhoenixKitDocumentCreator.Migrations do
  @moduledoc false

  defdelegate up(opts \\ []), to: PhoenixKitDocumentCreator.Migration
  defdelegate down(opts \\ []), to: PhoenixKitDocumentCreator.Migration
end

defmodule PhoenixKitDocumentCreator.Migration do
  @moduledoc """
  Versioned migrations for the Document Creator module.
  ...
  database version and only applies new migrations. On upgrades, just run:

      PhoenixKitDocumentCreator.Migration.up()
  ...
  @doc """
  Run migrations up to the latest (or specified) version.
  """
  def up(opts \\ []) do
  ...
  @doc """
  Roll back migrations. Without a `:version` option, rolls back completely.
  """
  def down(opts \\ []) do
    opts =
      opts
  ...
  @doc """
  Returns the current migrated version from the database.

  Reads the SQL comment on the `phoenix_kit_doc_headers_footers` table.
    ...
    case repo().query(table_exists_query, [], log: false) do
      {:ok, %{rows: [[true]]}} ->
        ...
        case repo().query(version_query, [], log: false) do
          ...
          # Table exists but no version comment — legacy pre-versioned install
          _ ->
            1
        end
  ...
  defp change(range, direction, opts) do
    Enum.each(range, fn index ->
      ...
      [PhoenixKitDocumentCreator.Migration.Postgres, "V#{pad}"]
      |> Module.concat()
