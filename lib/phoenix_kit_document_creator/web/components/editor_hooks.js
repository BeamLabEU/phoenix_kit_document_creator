...
(function() {
  ...
  //   Therefore 1mm = 96/25.4 = 3.7795px. Gotenberg (headless Chrome)
  //   uses the same 96 DPI standard, so the pixel dimensions here
  //   correspond 1:1 to the printed output.
  //
  //   Three unit systems are used across the stack:
  //     - JS (GrapesJS canvas):  CSS pixels (this file)
  //     - Elixir (Gotenberg):    inches for paperWidth/paperHeight
    ...
    return {
      mounted() {
        ...
        self.handleEvent("request-save-data", function() {
          if (!self._editor) {
            console.warn("[DocumentCreator] Save requested but editor not ready yet");
            self.pushEvent("editor_not_ready", {action: "save"});
            return;
          }
          var ed = self._editor;
          var data = {
        ...
        self.handleEvent("request-content-for-pdf", function(payload) {
          if (!self._editor) {
            console.warn("[DocumentCreator] PDF requested but editor not ready yet");
            self.pushEvent("editor_not_ready", {action: "pdf"});
            return;
          }
          var ed = self._editor;
          var html = ed.getHtml();
  ...
  window.PhoenixKitHooks.GrapesJSHFEditor = {
    mounted() {
      ...
      self.handleEvent("request-hf-save-data", function() {
        if (!self._editor) {
          console.warn("[DocumentCreator] HF save requested but editor not ready yet");
          self.pushEvent("editor_not_ready", {action: "save"});
          return;
        }
  ...
  // If liveSocket is already connected, disconnect and reconnect so LiveView
  // re-scans the DOM and attaches these newly-registered hooks to any
  // elements that were mounted before this script loaded.
  if (window.liveSocket) {
    var ls = window.liveSocket;
    if (ls.hooks) {
      ls.hooks.GrapesJSTemplateEditor = window.PhoenixKitHooks.GrapesJSTemplateEditor;
      ls.hooks.GrapesJSDocumentEditor = window.PhoenixKitHooks.GrapesJSDocumentEditor;
      ls.hooks.GrapesJSHFEditor = window.PhoenixKitHooks.GrapesJSHFEditor;
    }

    // Check if any hook elements exist but don't have their hook attached yet
    var needsReconnect = document.querySelector(
      '[phx-hook="GrapesJSTemplateEditor"]:not([data-phx-hook-attached]),' +
      '[phx-hook="GrapesJSDocumentEditor"]:not([data-phx-hook-attached]),' +
      '[phx-hook="GrapesJSHFEditor"]:not([data-phx-hook-attached])'
    );
    if (needsReconnect) {
      ls.disconnect(function() { ls.connect(); });
    }
  }
})();
