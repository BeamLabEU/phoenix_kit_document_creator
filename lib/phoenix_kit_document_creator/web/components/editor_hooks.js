...
(function() {
  ...
  function updatePageLayout(editor) {
    // If template page frame exists, delegate to HF-aware layout
    if (document.getElementById('template-page-frame')) {
  ...
  // The invariant is: headerH + bodyH + footerH = paperHeight (per page).
  // Separators are visual-only (overlaid) and do NOT reduce the body area.
  function updateTemplateEditorLayout(editor) {
    ...
    // Separators are visual-only overlays — not counted in the height budget
    var sepVisualH = 0;
    ...
    var bodyHeight = bodyPerPage * pages;

    // Page frame includes separators for display but they overlay, so add them
    var pageFrame = document.getElementById('template-page-frame');
    if (pageFrame) {
      pageFrame.style.height = (totalHeight + sepVisualH) + 'px';
    ...
    if (frame && frame.parentElement) {
      ...
      frame.parentElement.style.height = bodyHeight + 'px';
    }
    if (frame) frame.style.height = bodyHeight + 'px';

    var wrapper = editor.DomComponents.getWrapper();
    if (wrapper) wrapper.addStyle({ 'min-height': bodyHeight + 'px' });

    var canvas = editor.Canvas.getElement();
    if (canvas) canvas.style.height = bodyHeight + 'px';

    // Draw page dividers within body area at body-page intervals
    updatePageDividers(editor, { width: dims.width, height: bodyPerPage }, pages);
  }
  ...
  window.PhoenixKitHooks.GrapesJSTemplateEditor = {
    mounted() {
      ...
      ensureGrapesJS(function() {
        ...
        // The right panel (blocks) is placed beside the page frame in a
        // nested row so both start below the panels bar with no gap.
        editor.on('load', function() {
          var pageFrame = document.getElementById('template-page-frame');
          var rightPanel = document.getElementById('grapesjs-right-panel');
          var panelsEl = document.querySelector('#editor-grapesjs .gjs-pn-panels')
            || document.querySelector('#editor-grapesjs .gjs-pn-panel');
          if (panelsEl && pageFrame && pageFrame.parentElement) {
            ...
            // Column: panels bar on top, then row of page + blocks below
            var col = document.createElement('div');
            col.style.display = 'flex';
            col.style.flexDirection = 'column';
            col.style.width = '100%';
            // Row: page frame + right panel side by side
          ...
          requestAnimationFrame(function() { updatePageLayout(editor); });
        });
      });
      ...
      // Header/footer region updates from sidebar selection
  ...
  function initPageAwareHFEditor(opts) {
    ...
    bm.add('template-variable', { label: 'Variable {{ }}', category: 'Elements', content: '<p style="' + abs + 'color:#6b7280;background:#f3f4f6;padding:2px 6px;border-radius:4px;font-family:monospace;font-size:13px;display:inline-block;">{{ variable_name }}</p>' });
