defmodule PhoenixKitDocumentCreator.ActivityLogAssertions do
  @moduledoc """
  Helpers for asserting activity log entries landed with the right
  action, module, and metadata shape.

  Imported into `PhoenixKitDocumentCreator.DataCase` and
  `PhoenixKitDocumentCreator.LiveCase` so every DB-backed test can
  reach them.

  Mirrors the canonical shape from `phoenix_kit_locations` and
  ...
  ## Example
      ...
      assert_activity_logged("template.created",
        metadata_has: %{"name" => "Order Confirmation"}
      )
  """
